...
def textitout(entry)
  ...
  puts entry.creator.name
#  puts entry.deviceRecordedAt.name
#  puts entry.datetimeRecordedAt.name
#  puts entry.geospatialRecordedAt.name
end


get '/Artefact/:uuid' do
  if @entry = Artefact.find(params['uuid'])
    textitout(@entry)
    erb :artefact
  else
    "Could not find #{uuid}"
...
get '/AudioRecording/:uuid' do
  if @entry = AudioRecording.find(params['uuid'])
    erb :audiorecording
  else
    "Could not find #{uuid}"
  end
